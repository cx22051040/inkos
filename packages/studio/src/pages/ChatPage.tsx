...
import { getPortalEmptyGuidance } from "../features/portal";
import {
  DropdownMenu,
  DropdownMenuTrigger,
  DropdownMenuContent,
...
export function ChatPage({ activeBookId, mode = activeBookId ? "book" : "book-create", nav, theme, t, sse: _sse }: ChatPageProps) {
  ...
  const emptyGuidance = (() => {
    const portalGuidance = getPortalEmptyGuidance(currentSessionKind, isZh);
    if (portalGuidance) return portalGuidance;
    if (currentSessionKind === "short") {
      return isZh
        ? "说一个短篇方向、标题灵感、人物压力或核心冲突，我会使用短篇创作引擎生成正文、简介和封面。"
        : "Describe a short-fiction direction, title hook, pressure, or core conflict to use the short-fiction engine.";
    }
